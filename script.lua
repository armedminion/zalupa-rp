...
if gP:FindFirstChild("ZRP14") then gP.ZRP14:Destroy() end
for _,n in ipairs({"ZRP_Aim","ZRP_Spin","ZRP_AntiRag"}) do
    pcall(function() RS:UnbindFromRenderStep(n) end) end

local S = {
    ESP=false,Tracers=false,Speed=false,Noclip=false,SpeedVal=100,
    InfAmmo=false,NoRecoil=false,God=false,
    Fly=false,FlySpeed=80,
    Aim=false,AimFOV=350,AimStr=0.35,AimPart="Head",
...
--  GUI
-- ═══════════════════════════════════════
local SG=Instance.new("ScreenGui");SG.Name="ZRP14";SG.Parent=gP
...
local bG=Instance.new("UIGradient");bG.Parent=bdr
bG.Color=ColorSequence.new{ColorSequenceKeypoint.new(0,C.accent),ColorSequenceKeypoint.new(0.5,C.accent2),ColorSequenceKeypoint.new(1,C.accent)}
task.spawn(function() while MF and MF.Parent do for r=0,360,2 do bG.Rotation=r;task.wait(0.02) end end end)
...
local topL=Instance.new("Frame");topL.Parent=MF;topL.Size=UDim2.new(1,0,0,3);topL.BorderSizePixel=0;topL.BackgroundColor3=Color3.new(1,1,1)
Instance.new("UIGradient",topL).Color=ColorSequence.new{ColorSequenceKeypoint.new(0,C.accent),ColorSequenceKeypoint.new(0.5,C.accent2),ColorSequenceKeypoint.new(1,C.accent)}
...
su.Font=Enum.Font.Gotham;su.TextSize=9;su.Text="v14 • Xeno • mousemoverel ✅ • [H]";su.TextColor3=C.dim;su.TextXAlignment=Enum.TextXAlignment.Left
...
local dvF=Instance.new("Frame");dvF.Parent=MF;dvF.Position=UDim2.new(0.04,0,0,55)
dvF.Size=UDim2.new(0.92,0,0,1);dvF.BorderSizePixel=0;dvF.BackgroundColor3=Color3.fromRGB(40,40,70)
...
local function AntiRag(ch)
    ...
    for _,s in ipairs({Enum.HumanoidStateType.Ragdoll,Enum.HumanoidStateType.FallingDown,Enum.HumanoidStateType.Physics}) do
        pcall(function() hum:SetStateEnabled(s,false) end) end
    ...
    if st==Enum.HumanoidStateType.Ragdoll or st==Enum.HumanoidStateType.FallingDown or st==Enum.HumanoidStateType.Physics then
        pcall(function() hum:ChangeState(Enum.HumanoidStateType.GettingUp) end) end
    for _,o in ipairs(ch:GetDescendants()) do pcall(function()
...
local function nukeESP()
    for _,p in ipairs(Players:GetPlayers()) do if p.Character then
        for _,o in ipairs(p.Character:GetDescendants()) do
            if o.Name=="_E" or o.Name=="_EB" then pcall(function() o:Destroy() end) end end end end
end
...
Sep("Visuals")
Tog("ESP + Distance",function(on) S.ESP=on
    ...
    else dcESP();task.defer(nukeESP) end end)
...
Tog("Tracers",function(on) S.Tracers=on
    ...
    end) else clrT() end end)

-- ═══════════════════════════════════════
--  3. AIMBOT — PERFECT FOR XENO
--
--  mousemoverel(dx, dy) двигает мышь
--  dx * 0.35 = 35% расстояния за кадр
--  Экспоненциальное затухание:
--    100→65→42→27→18→12→8→5→3→2→1→done
--  Плавно, точно, без перелёта
-- ═══════════════════════════════════════
Sep("Combat")
...
Tog("Aimbot (RMB / Q)",function(on)
    ...
    if on then
        ...
        RS:BindToRenderStep("ZRP_Aim", Enum.RenderPriority.Input.Value, function()
            ...
            -- Расстояние от центра до цели в пикселях
            local dx = best.sp.X - cx
            local dy = best.sp.Y - cy

            -- Если уже на цели — не двигать
            if math.abs(dx) < 1 and math.abs(dy) < 1 then return end

            -- Двигаем мышь: 35% от оставшегося расстояния
            mousemoverel(dx * S.AimStr, dy * S.AimStr)
        end)
    else
        pcall(function() RS:UnbindFromRenderStep("ZRP_Aim") end)
...
local aId=0
Tog("Infinite Ammo",function(on) S.InfAmmo=on;aId=aId+1
...
local rId=0
Tog("No Recoil",function(on) S.NoRecoil=on;rId=rId+1
...
-- ═══════════════════════════════════════
--  6. GOD MODE (клиентский — лечение + блок смерти)
-- ═══════════════════════════════════════
local godId=0
Tog("God Mode (heal)",function(on)
    S.God=on;godId=godId+1
    if on then
        local id=godId
        task.spawn(function()
            while S.God and godId==id do
                pcall(function()
...
--  7. ANTI-RAGDOLL
-- ═══════════════════════════════════════
Tog("Anti-Ragdoll",function(on) S.AntiRag=on
    if on then
        ...
        RS:BindToRenderStep("ZRP_AntiRag",Enum.RenderPriority.Character.Value+5,function()
            if S.AntiRag then AntiRag(LP.Character) end end)
    else
        pcall(function() RS:UnbindFromRenderStep("ZRP_AntiRag") end)
...
-- ═══════════════════════════════════════
Sep("Movement")
Tog("Speed (x"..S.SpeedVal..")",function(on) S.Speed=on
...
LP.CharacterAdded:Connect(function(ch) task.wait(1)
    if S.Speed then pcall(function() ch:FindFirstChildOfClass("Humanoid").WalkSpeed=S.SpeedVal end) end end)

-- ═══════════════════════════════════════
--  9. NOCLIP
-- ═══════════════════════════════════════
local ncCn,ncSv=nil,{}
Tog("Noclip",function(on) S.Noclip=on
    if on then ncSv={};pcall(function() for _,p in ipairs(LP.Character:GetDescendants()) do
        if p:IsA("BasePart") then ncSv[p]=p.CanCollide end end end)
        ...
        task.defer(function()
            for pt,og in pairs(ncSv) do pcall(function() if pt and pt.Parent then pt.CanCollide=og end end) end;ncSv={}
...
Tog("Fly (WASD+Space+Shift)",function(on) if on then startFly() else stopFly() end end)
...
-- ═══════════════════════════════════════
Sep("Fun")
local spinB,spinG,spinA,origJP=false,nil,0,50

local function bindSpin()
    if spinB then return end;spinB=true
    ...
    RS:BindToRenderStep("ZRP_Spin",Enum.RenderPriority.Character.Value-1,function(dt)
        ...
        spinA=spinA+math.rad(S.SpinSpeed)*dt
        if not spinG or not spinG.Parent then
            spinG=Instance.new("BodyGyro");spinG.Name="_SG"
            spinG.MaxTorque=Vector3.new(0,1e6,0);spinG.P=1e6;spinG.D=100;spinG.Parent=hrp end
        spinG.CFrame=CFrame.new(hrp.Position)*CFrame.Angles(0,spinA,0)
...
local function unbindSpin() spinB=false
    pcall(function() RS:UnbindFromRenderStep("ZRP_Spin") end)
    if spinG then pcall(function() spinG:Destroy() end);spinG=nil end
    pcall(function() local ch=LP.Character;if not ch then return end;local hum=ch:FindFirstChildOfClass("Humanoid")
        if hum then hum.JumpPower=origJP;hum.AutoRotate=true
            hum:SetStateEnabled(Enum.HumanoidStateType.Ragdoll,true)
            hum:SetStateEnabled(Enum.HumanoidStateType.FallingDown,true)
            hum:ChangeState(Enum.HumanoidStateType.Running) end
        local hrp=ch:FindFirstChild("HumanoidRootPart")
        if hrp then local sg=hrp:FindFirstChild("_SG");if sg then sg:Destroy() end end end) end
Tog("Spinner + Bhop",function(on) S.Spin=on;if on then bindSpin() else unbindSpin() end end)
LP.CharacterAdded:Connect(function(ch) task.wait(1);if S.Spin then
    ...
    spinG=nil;unbindSpin();S.Spin=true;bindSpin() end end)

-- ═══════════════════════════════════════
--  12-13. FPS + REJOIN
-- ═══════════════════════════════════════
Sep("Other")
Act("FPS BOOST",function()
    ...
    for _,n in ipairs({"BloomEffect","BlurEffect","ColorCorrectionEffect","SunRaysEffect","DepthOfFieldEffect","Atmosphere"}) do
        ...
        pcall(function() for _,o in ipairs(workspace.CurrentCamera:GetChildren()) do if o:IsA(n) then o:Destroy() end end end) end
...
Act("Rejoin Server",function()
    pcall(function() SGui:SetCore("SendNotification",{Title="🔄",Text="Rejoining...",Duration=2}) end)
    task.wait(0.3)
...
UIS.InputBegan:Connect(function(i,g) if g then return end
    if i.KeyCode==Enum.KeyCode.H then MF.Visible=not MF.Visible end end)

pcall(function() SGui:SetCore("SendNotification",{
    Title="🍆 Zalupa RP v14",
    Text="Xeno optimized!\nmousemoverel ✅\nH=menu | RMB/Q=aim",Duration=7}) end)
